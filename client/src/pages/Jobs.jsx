...
export default function Jobs() {
  ...
  const activeJobs = jobs.filter(job=>!["Offer","Rejected"].includes(job.status)).length;
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-3 gap-3"><Summary label="Total" value={jobs.length}/><Summary label="Active" value={activeJobs}/><Summary label="Interviews" value={interviews}/></div>
...
function Summary({label,value}){return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-semibold text-slate-500">{label}</p><p className="mt-1 text-2xl font-extrabold text-slate-900">{value}</p></div>}
