...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([api.get("/courses"), api.get("/courses/mine"), api.get("/jobs")])
      .then(([c,e,j]) => { setCourses(c.data.courses); setEnrollments(e.data.enrollments); setJobs(j.data.jobs); });
  }, []);
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-7 lg:grid-cols-[1.5fr_1fr]">
      ...
      <div>
        <div className="mb-4"><h2 className="section-title">Recommended for you</h2><p className="mt-1 text-sm text-slate-500">Based on your career goal.</p></div>
        <div className="space-y-3">{courses.slice(0,2).map(course => <Link key={course._id} to={`/courses/${course._id}`} className="card block p-4 transition hover:border-brand-200"><div className="flex items-center gap-3"><img src={course.thumbnail} className="h-16 w-20 rounded-xl object-cover"/><div className="min-w-0"><p className="text-xs font-semibold text-brand-600">{course.category}</p><h3 className="mt-1 line-clamp-2 text-sm font-bold">{course.title}</h3><p className="mt-1 text-xs text-slate-500">{course.duration}</p></div></div></Link>)}</div>
      </div>
    </section>
  </div>;
}
