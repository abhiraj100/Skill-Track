...
import { ArrowRight, Award, BookOpen, BookMarked, BrainCircuit, BriefcaseBusiness, Flame, Medal, Target, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-7 lg:grid-cols-[1.5fr_1fr]">
      ...
      <div className="space-y-7"><StudyGoal summary={study} logging={logging} onLog={logStudyTime}/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1"><Link to="/mind-gym" className="block overflow-hidden rounded-2xl bg-gradient-to-br from-violet-700 to-cyan-600 p-5 text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-white/70"><BrainCircuit size={18}/><span className="text-xs font-bold tracking-wide">MIND GYM</span></div><h2 className="mt-2 text-lg font-extrabold">60-second brain break</h2><p className="mt-1 text-sm leading-6 text-white/80">Boost focus with a quick sprint.</p></div><ArrowRight className="mt-1"/></div></Link><Link to="/notes" className="block rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-amber-700"><BookMarked size={18}/><span className="text-xs font-bold tracking-wide">STUDY NOTES</span></div><h2 className="mt-2 text-lg font-extrabold">Capture key ideas</h2><p className="mt-1 text-sm leading-6 text-slate-600">Save takeaways for revision.</p></div><ArrowRight className="mt-1 text-amber-700"/></div></Link><Link to="/achievements" className="block rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 p-5 text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-white/75"><Medal size={18}/><span className="text-xs font-bold tracking-wide">ACHIEVEMENTS</span></div><h2 className="mt-2 text-lg font-extrabold">Collect your badges</h2><p className="mt-1 text-sm leading-6 text-white/85">Turn learning actions into milestones.</p></div><ArrowRight className="mt-1"/></div></Link></div><div><div className="mb-4"><h2 className="section-title">Recommended for you</h2><p className="mt-1 text-sm text-slate-500">Based on your career goal.</p></div><div className="space-y-3">{courses.slice(0,2).map(course => <Link key={course._id} to={`/courses/${course._id}`} className="card block p-4 transition hover:border-brand-200"><div className="flex items-center gap-3"><img src={course.thumbnail} className="h-16 w-20 rounded-xl object-cover"/><div className="min-w-0"><p className="text-xs font-semibold text-brand-600">{course.category}</p><h3 className="mt-1 line-clamp-2 text-sm font-bold">{course.title}</h3><p className="mt-1 text-xs text-slate-500">{course.duration}</p></div></div></Link>)}</div></div></div>
    </section>
  </div>;
}
