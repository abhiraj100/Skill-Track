...
export const register = async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      ...
      user: { id: user._id, name: user.name, email: user.email, role: user.role, careerGoal: user.careerGoal, skills: user.skills }
    });
  } catch (e) { next(e); }
};
...
export const updateProfile = async (req, res, next) => {
  try {
    const user = await User.findByIdAndUpdate(
      req.user._id,
      { name: req.body.name, careerGoal: req.body.careerGoal, skills: req.body.skills || [] },
      { new: true }
    ).select("-password");
